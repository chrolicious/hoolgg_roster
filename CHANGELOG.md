...
- Test release to validate auto-update flow from v0.1.9

## [0.1.9] - 2026-01-31

### 🐛 Bug Fixes

- **Actually disable signature validation**: validateUpdate didn't control NsisUpdater's signature check — overrode _verifyUpdateCodeSignature directly
...
- **Disable update signature validation**: Installers are not code-signed; electron-updater was rejecting updates
...
- **Auto-download enabled**: Updates now download automatically in the background — no button click required
- This ensures users are never stuck on an old version even if the UI has issues
...
- **In-App Update UI**: View current version and update status in Settings
- **Manual Update Check**: Check for updates without restarting the app
- **Update Notifications**: Real-time update status in the app interface

### 🔧 CI/CD

- **Automated Releases**: Fixed permissions for automated GitHub releases
- **Complete Pipeline**: Validated end-to-end CI/CD with Python bundling
